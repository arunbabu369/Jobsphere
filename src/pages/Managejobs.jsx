...
return (
    <div className='bg-white rounded-xl p-8 mx-auto max-w-4xl mt-14 sm:my-8'>
      <h1 className='text-2xl sm:text-3xl font-inter text-center mb-6'>Manage Your Posted Jobs</h1>
      ...
      (<div className='mt-2 sm:mt-14 space-y-2 sm:space-y-8'>
        {employerjobs.map(job=>(
            <div key={job.id} className='border p-4 rounded-lg shadow-sm flex flex-col   md:flex-row justify-between space-y-4 md:space-y-0 items-center'>
                <div className='text-lg font-semibold mb-2 md:mb-0'>
                    {job.title} at {job.company}
                    <p className='ext-gray-600 text-sm'>{job.location}-{job.type}</p>
                </div>
                <div className='flex space-x-2'>
                    <button className='bg-indigo-700 rounded-md p-1 sm:p-2 text-sm text-white hover:bg-indigo-600' onClick={()=>handleApplicants(job.id)}>View Applicants
                        </button>
                    <button className='bg-blue-700 rounded-md  p-1 sm:p-2 text-sm text-white' onClick={()=>handleDelete(job.id)}>Delete</button>
               
                    <button className='bg-green-700 rounded-md py-1 px-2 sm:px-4  text-sm  sm:py-2 text-white' onClick={()=>handleEdit(job.id)}>Edit</button>
                </div>
            </div>
        ))} 
      </div>)}
