 ...
 const handleUnsave=async(unsaveid,jobtitle)=>{
  ...
  return (
    <div className='mx-auto px-8 py-4'>
        <h1 className='text-center text-3xl sm:text-4xl font-semibold text-indigo-700 my-10'>Saved Jobs</h1>
        {savejobs.length==0?(
            <div className='text-xl sm:text-3xl p-6 text-center '>
        ...
        (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">{savejobs.map(job=>(
            <div key={job.id} className='bg-white rounded-lg p-2 sm:p-6 shadow-lg border border-gray-100'>
                ...
                <button onClick={()=>handleUnsave(job.id,job.title)} className='bg-blue-700 text-white font-semibold text-center text-sm sm:text-base mx-16 sm:mx-20 px-4 py-1 rounded-2xl'>Unsave</button>
            </div>
