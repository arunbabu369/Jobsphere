...
function Editjob() {
    ...
    const {refreshjobs,updatedJob,jobs,loadingjobs}=useContext(Jobcontext)
    ...
    useEffect(()=>{
        if(!loadingjobs && jobs.length>0 &&initialLoad){
            ...
            if(jobtoEdit){
                ...
                setjobsdata({
                    ...
                    remoteHybrid: jobtoEdit.remoteHybrid || 'Hybrid', 
            ...
            console.error("Job not found in Localstorage")
            navigate('/employer/Manage-Jobs')
            }
        }
    },[jobs,loadingjobs,jobId,currentuser,initialLoad])
   ...
   const handleSubmit=(e)=>{
        ...
        if (!currentuser || currentuser.role !== 'employer') {
            setError(prev => ({ ...prev, general: 'You must be logged in as an employer to update jobs.' }));
            return;
        }
        if(validateForm()){
            const updatedjob={
            ...
            if(success){
                ...
                setError(prev => ({ ...prev, general: 'Failed to update job. Please try again.' }));
            }
        }
        if(loadingjobs){
       ...
       if(!currentuser||currentuser.role!=='employer'){
         return <div className="text-center py-8 text-red-600">You must be logged in as an employer to manage jobs.</div>;
       }
        }
  return (
    <div className='bg-white rounded-xl p-8 mx-auto'>
      <h1 className='text-2xl font-inter text-center'>Edit Job</h1>
